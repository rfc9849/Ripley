...
Future<void> main() async {
  ...
  try {
    final png = base64Decode(
      'iVBORw0KGgoAAAANSUhEUgAAAEAAAABACAYAAACqaXHeAAAAaklEQVR42u3QoQ0AQAwDsey/9BeXPK/ik8KCnFdeAAAAAAAAAAAAAAAA8D8ma9f+AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAIC+AAAAAAAAAAAAAAAobQCiDUrJ145lxgAAAABJRU5ErkJggg==',
    );
    final file = File('${Directory.systemTemp.path}/ripley_mlkit_smoke.png');
    await file.writeAsBytes(png, flush: true);
    final result = await recognizer.processImage(InputImage.fromFilePath(file.path));
    stdout.writeln('MLKIT_SMOKE_OK text=${result.text.length}');
  } finally {
...
class PipelineCheckApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'Ripley Pipeline Check ${TextRecognitionScript.latin.name}',
      theme: ThemeData(
